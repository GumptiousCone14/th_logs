...
exports('discord', function(message, id, id2, color, channel)
	...
	if message == nil then print("^1Error: th_logs Export. Invalid message.^0") return end
	if id == nil or id == "PLAYER_ID" or not tonumber(id) then print("^1Error: th_logs Export. Invalid player id.^0") return end
	if id == nil or id2 == "PLAYER_2_ID" or not tonumber(id2) then print("^1Error: th_logs Export. Invalid second player id.^0") return end
	if color == nil then print("^1Error: th_logs Export. Invalid color.^0") return end
	if channel == nil or channel == "" then print("^1Error: th_logs Export. Invalid channel.^0") return end
...
function HidePlayerDetails(message, color, channel)
	if Config.AllLogs then
		PerformHttpRequest(Config.webhooks["all"], function(err, text, headers) getStatus(err,"all") end, 'POST', json.encode({
  	...
  	PerformHttpRequest(Config.webhooks[channel], function(err, text, headers) getStatus(err,channel) end, 'POST', json.encode({
		username = Config.username, 
		embeds = {{
			["color"] = color, 
			["author"] = {
...
function PluginHidePlayerDetails(message, color, channel)
	...
	if Config.AllLogs then
  	...
  	PerformHttpRequest(Config.Plugins[channel].webhook, function(err, text, headers) getStatus(err,channel) end, 'POST', json.encode({
		username = Config.username, 
		embeds = {{
			["color"] = _color, 
			["author"] = {
...
function getStatus(status, channel)
	if status == 404 or status == 401 and Config.webhooks[channel] ~= "DISCORD_WEBHOOK" and Config.webhooks[channel] ~= "" then 
		print('^3Warn: th_logs webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end

-- version check
Citizen.CreateThread( function()
		SetConvarServerInfo("th_logs", "V"..Config.versionCheck)
...
^0]]):format(
						...
						print('th_logs unable to check version')
					end
				end,
				'GET'
			)
